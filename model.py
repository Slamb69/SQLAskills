...
class Brand(db.Model):
    ...
    # create the db columns based on the cars db.
...
class Model(db.Model):
    ...
    # Define a relationship w/Brand, backref to models via brand_id foreign key.
    brand = db.relationship('Brand', backref='models')

    # define repr function to print some useful info re:db objects.
    def __repr__(self):
        """Print more useful info."""
        return "<Model id=%d brand_id=%s name=%s year=%d>" % (self.model_id,
                                                              self.brand_id,
                                                              self.name,
                                                              self.year)
